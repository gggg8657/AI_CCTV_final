...
### 5.3 Phase 3: 얼굴 인식 기능 추가 (2-3주)

**목표**: 방문자 얼굴 인식 및 추적

**작업:**
1. 얼굴 인식 모델 통합
   - InsightFace 또는 FaceNet 선택
   - 얼굴 임베딩 생성
   - 얼굴 데이터베이스 구축
...
4. Function Calling 함수 추가
   - `get_visitor_count()`: 방문자 수 조회
   - `get_visitor_details()`: 방문자 상세 정보
   - `remember_my_face(name)`: 얼굴 등록
   - `get_known_faces()`: 등록된 얼굴 목록
